...
import { Authorize, CheckAuthorization, getAccessToken } from "@/igdb/auth";
import { Tabs, Tab } from "@nextui-org/tabs";

var auth = require("@/igdb/auth");

const GamePage = async ({ params }: { params: { slug: string } }) => {
  const { slug } = params;
  console.log("WE ARE THE SLUG " + slug);
...
const GetGame = async (slug: string): Promise<Game> => {
  const access_token = "07u1ipwv2feel64wmmez8torrn7s0h";
  console.log(
  ...
  const response = await fetch("https://api.igdb.com/v4/games", {
    ...
    body: `fields *, keywords.name, cover.*, artworks.*, genres.*, involved_companies.*; where slug = ${slug}; limit 1;`,
  });
  console.log("Slug page response status: " + (await response.status));
  // Authorization Error
